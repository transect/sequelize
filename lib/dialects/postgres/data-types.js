...
module.exports = BaseTypes => {
  ...
  const SUPPORTED_DIMENSIONALITIES = ['2D', '3D', '3DZ', '3DM', '4D'];

  function GEOMETRY(type, srid, dimensionality) {
    if (!(this instanceof GEOMETRY)) return new GEOMETRY(type, srid);
    BaseTypes.GEOMETRY.apply(this, arguments);

    const options = _.isPlainObject(type) ? type : {type, srid, dimensionality};
    this.options = options;
    this.dimensionality = options.dimensionality || '2D';

    if (!_.isEmpty(this.type) && !_.includes(SUPPORTED_DIMENSIONALITIES, this.dimensionality)) {
      throw new Error('Supported dimensionalities are: ' + SUPPORTED_DIMENSIONALITIES.join(', '));
    }
  }
  ...
  GEOMETRY.prototype._stringify = function _stringify(value, options) {
    let stringified = 'ST_GeomFromGeoJSON(' + options.escape(JSON.stringify(value)) + ')';

    if (this.srid) {
      stringified = 'ST_SetSRID(' + stringified + ', 4326)';
    }

    if (this.dimensionality) {
      stringified = 'ST_Force' + this.dimensionality + '(' + stringified + ')';
    }
